...
public class enterLevel : MonoBehaviour {
	...
	public void SetLevel(int level)
	{
		...
		if (tempLGS.levelID == 21)
			levelIndicator.GetComponent<Text>().text = "5-5";
		else
			levelIndicator.GetComponent<Text>().text = (((tempLGS.levelID - 1) / 4) + 1) + "-" + (tempLGS.levelID - (((tempLGS.levelID - 1) / 4) * 4));
		descriptionText.GetComponent<Text>().text = tempLGS.description;
		highscoreIndicator.GetComponent<Text>().text = PlayerPrefs.GetInt("ppTopScoreLevel" + tempLGS.levelID.ToString()).ToString();
		targetScore.text = tempLGS.scoreBonusAmt.ToString();

		ScoringSystemStar.SCORING_TYPES tempRank = (ScoringSystemStar.SCORING_TYPES)PlayerPrefs.GetInt("ppTopRankLevel" + tempLGS.levelID, (int)ScoringSystemStar.SCORING_TYPES.END_FALSE_LIFE_FALSE_SCORE_FALSE);
		switch (tempRank)
		{
		case ScoringSystemStar.SCORING_TYPES.END_TRUE_LIFE_TRUE_SCORE_TRUE:
			rankIndicator[0].GetComponent<Image>().color = medalUnlockedColor;
